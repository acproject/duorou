...
SentencePieceProcessor::SentencePieceProcessor(
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    ...
    case TOKEN_TYPE_UNUSED:
      max_token_len_ =
          std::max(max_token_len_, static_cast<int>(values[i].length()));
      break;
    default:
      break;
    }
...
std::vector<int32_t> SentencePieceProcessor::encode(const std::string &text,
                                                    bool add_special) {
  // Input validation
  if (text.empty()) {
    std::cerr << "[SentencePieceProcessor] Warning: Empty input text"
              << std::endl;
    return {};
  }

  if (text.length() > 100000) {
    std::cerr << "[SentencePieceProcessor] Warning: Input text too long ("
              << text.length() << " chars), truncating" << std::endl;
  }
  ...
  try {
    ...
    try {
      ...
      std::cerr << "[SentencePieceProcessor] Special token processing failed: "
                << e.what() << ", using fallback" << std::endl;
      return characterLevelFallback(text);
    }
    ...
    for (const auto &frag : fragments) {
      ...
      try {
        ...
        try {
          fragment_ids = tokenizeFragment(processed_text);
        } catch (const std::exception &e) {
          std::cerr << "[SentencePieceProcessor] Fragment tokenization failed: "
                    << e.what() << ", using fallback for fragment" << std::endl;
          fragment_ids = characterLevelFallback(frag.value);
        }

        if (fragment_ids.empty()) {
          std::cerr << "[SentencePieceProcessor] Warning: Fragment "
                       "tokenization returned empty result, using fallback"
                    << std::endl;
          fragment_ids = characterLevelFallback(frag.value);
        }
        ...
        std::cerr << "[SentencePieceProcessor] Fragment processing failed: "
                  << e.what() << ", using fallback" << std::endl;
        std::vector<int32_t> fallback_ids = characterLevelFallback(frag.value);
        ids.insert(ids.end(), fallback_ids.begin(), fallback_ids.end());
      }
    }

    // Final validation
    if (ids.empty() && !text.empty()) {
      std::cerr << "[SentencePieceProcessor] Warning: No tokens generated, "
                   "using character-level fallback"
                << std::endl;
      return characterLevelFallback(text);
    }
    ...
    if (add_special && !ids.empty()) {
      try {
        ...
        std::cerr << "[SentencePieceProcessor] Adding special tokens failed: "
                  << e.what() << std::endl;
      }
    }

  } catch (const std::exception &e) {
    std::cerr << "[SentencePieceProcessor] Encoding failed: " << e.what()
              << ", using character-level fallback" << std::endl;
    return characterLevelFallback(text);
  }
...
size_t SentencePieceProcessor::getVocabSize() const { return vocab_->size(); }

std::vector<Fragment>
SentencePieceProcessor::processSpecialTokens(const std::string &text) const {
...
std::vector<int32_t>
SentencePieceProcessor::tokenizeFragment(const std::string &text) const {
  ...
  auto createCandidate = [&](int a, int b) -> std::shared_ptr<Candidate> {
    ...
    if (token_id >= 0) {
      ...
      float score = (token_id < static_cast<int32_t>(scores.size()))
                        ? scores[token_id]
                        : 0.0f;
      return std::make_shared<Candidate>(a, b, score,
                                         static_cast<int>(combined.length()));
    }
  ...
  for (size_t i = 0; i < runes.size() - 1; ++i) {
    auto candidate =
        createCandidate(static_cast<int>(i), static_cast<int>(i) + 1);
    if (candidate) {
  ...
  while (!queue.empty()) {
    ...
    if (left.runes.empty() || right.runes.empty() ||
        static_cast<int>(runesToString(left.runes).length() +
                         runesToString(right.runes).length()) !=
            candidate->size) {
      continue;
    }
      ...
      if (token_id >= 0) {
        ...
        for (char c : token_str) {
          ...
          ss << "<0x" << std::hex << std::uppercase << std::setfill('0')
             << std::setw(2) << static_cast<unsigned char>(c) << ">";
...
std::vector<char32_t>
SentencePieceProcessor::stringToRunes(const std::string &text) const {
...
std::string SentencePieceProcessor::runesToString(
    const std::vector<char32_t> &runes) const {
  std::string result;
...
std::vector<int32_t>
SentencePieceProcessor::characterLevelFallback(const std::string &text) const {
  std::vector<int32_t> result;

  try {
    // Convert text to Unicode runes
    std::vector<char32_t> runes = stringToRunes(text);

    for (char32_t rune : runes) {
      // Try to encode single character
      std::string char_str = runesToString({rune});
      int32_t token_id = vocab_->encode(char_str);

      if (token_id != -1) {
        result.push_back(token_id);
      } else {
        // Try with whitespace separator prefix for space-like characters
        if (rune == U' ') {
          int32_t ws_id = vocab_->encode(WHITESPACE_SEP);
          if (ws_id != -1) {
        ...
        // Try byte-level encoding for non-ASCII characters
        std::string utf8_bytes = char_str;
        bool byte_encoded = false;

        for (unsigned char byte : utf8_bytes) {
          std::string byte_token = "<0x" + std::to_string(byte) + ">";
          int32_t byte_id = vocab_->encode(byte_token);
          if (byte_id != -1) {
            result.push_back(byte_id);
            byte_encoded = true;
          }
        }

        if (!byte_encoded) {
          // Last resort: use UNK token
          int32_t unk_id = vocab_->encode("<unk>");
          if (unk_id != -1) {
            result.push_back(unk_id);
          } else {
            // Absolute fallback: use token ID 0
            result.push_back(0);
          }
        }
      }
