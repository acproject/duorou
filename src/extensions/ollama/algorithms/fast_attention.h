...
public:
  ...
  bool initialize(const ModelConfig &config,
    ...
    log("INFO",
        "FastAttention initialized with head_dim=" + std::to_string(head_dim_) +
            ", num_heads=" + std::to_string(num_heads_));

    return true;
  }

  std::string getName() const override { return "FastAttention"; }
  ...
  Tensor compute(const Tensor &query, const Tensor &key, const Tensor &value,
    ...
    if (actual_head_dim == 0 || actual_head_dim > 4096) {
      throw std::invalid_argument("Invalid head dimension: " +
                                  std::to_string(actual_head_dim));
    }

    // 确保key和value的维度与query匹配
    if (key.shape.back() != actual_head_dim ||
        value.shape.back() != actual_head_dim) {
      throw std::invalid_argument(
          "Dimension mismatch: query=" + std::to_string(actual_head_dim) +
    ...
    output.data.assign(output_buffer.begin(),
                       output_buffer.begin() + seq_len_q * actual_head_dim);
    ...
    if (seq_len_k == 1) {
      ...
      computeFastSingleKeyAttention(query, key, value, output, effective_scale,
                                    mask, actual_head_dim);
    } else if (seq_len_q == 1 && seq_len_k > 1) {
      // 增量解码模式
      computeIncrementalAttention(query, key, value, output, effective_scale,
                                  mask, actual_head_dim);
    } else {
      // 标准注意力计算
  ...
  Tensor computeWithCache(const Tensor &query, const Tensor &key,
    ...
    if (!validateInput(query) || !validateInput(key) || !validateInput(value)) {
      throw std::invalid_argument(
          "Invalid input tensors for FastAttention computeWithCache");
    }
  ...
  void updateKVCache(const Tensor &key, const Tensor &value, Tensor &key_cache,
    ...
    // 判断缓存是3D还是4D
    if (key_cache.shape.size() == 4) {

      // 4D存储布局为[B, kv_heads, T, head_dim]
      ...
      if (head_dim != cache_head_dim) {
        throw std::runtime_error(
            "Head dimension mismatch: key=" + std::to_string(head_dim) +
            ", cache=" + std::to_string(cache_head_dim));
      }

      if (cache_position >= max_seq_len) {
        throw std::runtime_error(
            "Cache position " + std::to_string(cache_position) +
    ...
    } else if (key_cache.shape.size() == 3) {
      // 3D缓存布局为[B , T, head_dim]
      uint32_t batch_size = key_cache.shape[0];
      uint32_t max_seq_len = key_cache.shape[1];
      uint32_t cache_head_dim = key_cache.shape[2];
      ...
      // 计算在3D缓存中的偏移量
      uint32_t cache_offset = cache_position * head_dim;
...
private:
  ...
  bool isCacheHit(const Tensor &key, const Tensor &value,
    ...
    const uint32_t check_range =
        std::min(cache_position, 3u); // 检查最近3个位置

    for (uint32_t pos = cache_position - check_range; pos < cache_position;
         ++pos) {
      bool key_match = true;
      ...
      for (uint32_t i = 0; i < head_dim && value_match; ++i) {
        float diff =
            std::abs(value.data[i] - value_cache.data[pos * head_dim + i]);
        if (diff > tolerance) {
          value_match = false;
        }
      }
  ...
  // 特殊优化：当key/value只有一个token时的快速路径
  void computeFastSingleKeyAttention(const Tensor &query, const Tensor &key,
                                     const Tensor &value, Tensor &output,
                                     float scale, const Tensor *mask,
                                     uint32_t head_dim) {
    std::cerr << "[DEBUG] FastAttention::computeFastSingleKeyAttention called "
                 "with seq_len_q="
              << query.shape[query.shape.size() - 2]
              << ", seq_len_k=1, head_dim=" << head_dim << std::endl;

    uint32_t seq_len_q = query.shape[query.shape.size() - 2];

    // 检查数据大小
    if (query.data.size() < seq_len_q * head_dim ||
        key.data.size() < head_dim || value.data.size() < head_dim ||
        output.data.size() < seq_len_q * head_dim) {
      throw std::runtime_error("Tensor data size insufficient for fast single "
    ...
    std::cerr << "[DEBUG] Fast single key attention computation completed"
              << std::endl;
  }

  void computeStandardAttention(const Tensor &query, const Tensor &key,
                                const Tensor &value, Tensor &output,
                                float scale, const Tensor *mask,
                                uint32_t head_dim) {
    // 边界检查
    if (query.shape.empty() || key.shape.empty() || value.shape.empty() ||
        output.shape.empty()) {
      throw std::invalid_argument(
          "Invalid tensor shapes in computeStandardAttention");
    }
    ...
    if (query.data.size() < seq_len_q * head_dim ||
        ...
        output.data.size() < seq_len_q * head_dim) {
      throw std::runtime_error(
          "Tensor data size insufficient for attention computation");
    }
    ...
    for (uint32_t i = 0; i < seq_len_q; ++i) {
      ...
      for (uint32_t j = 0; j < seq_len_k; ++j) {
        ...
        // 4路展开优化
        for (; d + 3 < head_dim; d += 4) {
          score += q_row[d] * k_row[d] + q_row[d + 1] * k_row[d + 1] +
                   q_row[d + 2] * k_row[d + 2] + q_row[d + 3] * k_row[d + 3];
        }
  ...
  void computeIncrementalAttention(const Tensor &query, const Tensor &key,
    ...
    if (query.shape.empty() || key.shape.empty() || value.shape.empty() ||
        output.shape.empty()) {
      throw std::invalid_argument(
          "Invalid tensor shapes in computeIncrementalAttention");
    }
