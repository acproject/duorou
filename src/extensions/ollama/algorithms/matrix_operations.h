...
public:
  ...
  std::string getName() const override { return "MatrixOperations"; }
  ...
  void multiply(const float *a, const float *b, float *c, size_t m, size_t n,
                size_t k) override {
    ...
    // 添加维度检查
    if (m == 0 || n == 0 || k == 0) {
      std::cerr << "[ERROR] Invalid matrix dimensions: m=" << m << ", n=" << n
                << ", k=" << k << std::endl;
      throw std::invalid_argument("Invalid matrix dimensions");
    }

    multiplyStandard(a, b, c, m, n, k);
    std::cerr << "[DEBUG] MatrixOperations::multiply completed successfully"
              << std::endl;
  }
  ...
  void vectorMul(const float *a, const float *b, float *result,
                 size_t size) override {
    for (size_t i = 0; i < size; ++i) {
      result[i] = a[i] * b[i];
    }
...
protected:
  ...
  void multiplyStandard(const float *a, const float *b, float *c, size_t m,
                        size_t n, size_t k) {
...
public:
  OptimizedMatrixOperations() {
    ...
    std::cerr << "[INFO] OptimizedMatrixOperations initialized with "
              << num_threads_ << " threads, SIMD width: " << SIMD_WIDTH
              << std::endl;
  }
  virtual ~OptimizedMatrixOperations() = default;

  std::string getName() const override { return "OptimizedMatrixOperations"; }

  void multiply(const float *a, const float *b, float *c, size_t m, size_t n,
                size_t k) override {
    std::cerr << "[DEBUG] OptimizedMatrixOperations::multiply called with "
                 "dimensions: "
              << "m=" << m << ", n=" << n << ", k=" << k << std::endl;
    ...
    std::cerr << "[DEBUG] OptimizedMatrixOperations::multiply completed"
              << std::endl;
  }
...
private:
  ...
  void multiplyOptimized(const float *a, const float *b, float *c, size_t m,
                         size_t n, size_t k) {
  ...
  void multiplySIMD(const float *a, const float *b, float *c, size_t m,
                    size_t n, size_t k) {
...
#if defined(USE_AVX2) || defined(USE_AVX)
  void multiplySIMD_AVX(const float *a, const float *b, float *c, size_t m,
                        size_t n, size_t k) {
    // 初始化输出矩阵
    std::memset(c, 0, m * n * sizeof(float));
...
#if defined(USE_NEON)
  void multiplySIMD_NEON(const float *a, const float *b, float *c, size_t m,
                         size_t n, size_t k) {
    // 初始化输出矩阵
    std::memset(c, 0, m * n * sizeof(float));
  ...
  void multiplyParallel(const float *a, const float *b, float *c, size_t m,
                        size_t n, size_t k) {
    // 初始化输出矩阵
    std::memset(c, 0, m * n * sizeof(float));
    ...
    if (memory_pool_) {
      ...
      for (unsigned int t = 0; t < num_threads_; ++t) {
        ...
        size_t end_row = (t == num_threads_ - 1)
                             ? m
                             : std::min(m, (t + 1) * rows_per_thread);
    ...
    for (unsigned int t = 0; t < num_threads_; ++t) {
      size_t start_row = t * rows_per_thread;
      size_t end_row =
          (t == num_threads_ - 1) ? m : std::min(m, (t + 1) * rows_per_thread);

      if (start_row >= end_row)
        break;

      std::vector<float> *temp_buffer =
          memory_pool_ ? temp_buffers[buffer_idx++] : nullptr;
...
public:
  explicit BlockMatrixOperations(size_t block_size = 64)
      : block_size_(block_size) {}
  virtual ~BlockMatrixOperations() = default;

  std::string getName() const override { return "BlockMatrixOperations"; }
...
private:
  ...
  void multiplyBlocked(const float *a, const float *b, float *c, size_t m,
                       size_t n, size_t k) {
    // 简化的分块实现
    for (size_t i = 0; i < m; i += block_size_) {
      for (size_t j = 0; j < n; j += block_size_) {
        for (size_t l = 0; l < k; l += block_size_) {
          ...
          multiplyBlock(a + i * k + l, b + l * n + j, c + i * n + j, block_m,
                        block_n, block_k, k, n, n);
        }
      }
    }
  }

  void multiplyBlock(const float *a, const float *b, float *c, size_t m,
