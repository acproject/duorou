...
bool Vocabulary::is(int32_t id, Special special) const {
  switch (special) {
  case SPECIAL_BOS:
    return std::find(bos_tokens_.begin(), bos_tokens_.end(), id) !=
           bos_tokens_.end();
  case SPECIAL_EOS:
    return std::find(eos_tokens_.begin(), eos_tokens_.end(), id) !=
           eos_tokens_.end();
  default:
    return false;
  }
...
Vocabulary::addSpecials(const std::vector<int32_t> &ids) const {
  ...
  if (add_bos_ && !bos_tokens_.empty()) {
    ...
    if (std::find(bos_tokens_.begin(), bos_tokens_.end(), result[0]) ==
        bos_tokens_.end()) {
  ...
  if (add_eos_ && !eos_tokens_.empty()) {
    ...
    if (std::find(eos_tokens_.begin(), eos_tokens_.end(), result.back()) ==
        eos_tokens_.end()) {
...
void Vocabulary::buildSpecialVocabCache() const {
  special_vocab_.clear();
  for (size_t i = 0; i < values_.size(); ++i) {
    if (types_[i] == TOKEN_TYPE_CONTROL ||
...
createTextProcessor(const std::string &type, std::shared_ptr<Vocabulary> vocab,
  ...
  if (type == "sentencepiece") {
    ...
    // For now, return nullptr until llama.cpp integration is complete
    return nullptr;
  }
  return nullptr;
}

// Factory function to create text processor with llama.cpp vocabulary from
// model path std::unique_ptr<TextProcessor> createLlamaTextProcessor(const
// std::string& model_path) {
//     auto adapter = std::make_shared<LlamaVocabAdapter>();
//     if (adapter->initializeWithModel(model_path)) {
//         return std::make_unique<LlamaTextProcessor>(adapter);
//     }
//     return nullptr;
// }

// Factory function to create text processor with existing llama vocabulary
// std::unique_ptr<TextProcessor> createLlamaTextProcessor(const llama_vocab*
// vocab) {
//     auto adapter = std::make_shared<LlamaVocabAdapter>();
//     if (adapter->initializeWithVocab(vocab)) {
//         return std::make_unique<LlamaTextProcessor>(adapter);
//     }
//     return nullptr;
// }
