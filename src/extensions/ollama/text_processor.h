...
// Forward declarations for llama.cpp integration (commented out for now)
// struct llama_vocab;
// struct llama_model;
...
// Forward declaration of LlamaVocabAdapter (commented out for now)
...
struct Fragment {
  ...
  Fragment(const std::string &v, const std::vector<int32_t> &i)
      : value(v), ids(i) {}
};

// Special token types
enum Special { SPECIAL_BOS = 0, SPECIAL_EOS };
...
// Factory function to create text processor with llama.cpp vocabulary
// (commented out for now) std::unique_ptr<TextProcessor>
// createLlamaTextProcessor(const std::string& model_path);

// Factory function to create text processor with existing llama vocabulary
// (commented out for now) std::unique_ptr<TextProcessor>
// createLlamaTextProcessor(const llama_vocab* vocab);
