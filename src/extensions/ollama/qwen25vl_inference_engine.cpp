...
Qwen25VLInferenceEngine::Qwen25VLInferenceEngine()
    : config_(),
      ...
      vision_encoder_(nullptr),
      bos_token_id_(151643),
      eos_token_id_(151645),
      pad_token_id_(151643),
      unk_token_id_(151643),
      kv_cache_(nullptr),
      kv_cache_enabled_(false),
      temperature_(1.0f),
      top_p_(0.9f),
      top_k_(50),
      repetition_penalty_(1.1f),
      model_loaded_(false),
      verbose_(false),
      max_sequence_length_(2048),
      num_threads_(1),
...
Qwen25VLInferenceEngine::Qwen25VLInferenceEngine(bool verbose)
    : config_(),
      ...
      total_inference_time_(0.0),
      total_tokens_generated_(0) {
    log("INFO", "Qwen25VLInferenceEngine initialized with verbose=" + std::to_string(verbose));
}
...
bool Qwen25VLInferenceEngine::loadModel(const std::string& model_path) {
    ...
    try {
        ...
        kv_cache_->resize(config_.num_layers, max_sequence_length_, config_.hidden_size);
...
// 检查模型是否已加载
bool Qwen25VLInferenceEngine::isModelLoaded() const {
    return model_loaded_;
}

// 生成文本
std::string Qwen25VLInferenceEngine::generateText(const std::string& prompt, int max_tokens) {
    std::cout << "[DEBUG] Qwen25VLInferenceEngine::generateText called with prompt: " << prompt.substr(0, 20) << "..." << std::endl;
    
    if (!model_loaded_) {
        std::cout << "[DEBUG] Model not loaded, returning empty string" << std::endl;
        log("ERROR", "Model not loaded");
        return "";
    }
    
    std::cout << "[DEBUG] Model is loaded, starting text generation" << std::endl;
    log("INFO", "Generating text for prompt: " + prompt + ", max_tokens: " + std::to_string(max_tokens));
    ...
    std::cout << "[DEBUG] Tokenization completed, tokens: " << input_tokens.size() << std::endl;
    log("DEBUG", "Tokenization completed, tokens: " + std::to_string(input_tokens.size()));
    ...
    for (int i = 0; i < max_tokens; ++i) {
        ...
        std::cout << "[DEBUG] Calling forward() with " << input_tokens.size() << " tokens" << std::endl;
        ...
        // 检查多种可能的EOS token
        if (next_token == eos_token_id_ || next_token == 151935 || next_token == 151643) {
            std::cout << "[DEBUG] EOS token encountered: " << next_token << " (configured eos_token_id_: " << eos_token_id_ << ")" << std::endl;
...
// 带图像的文本生成
std::string Qwen25VLInferenceEngine::generateTextWithImage(const std::string& prompt, const std::string& image_path, int max_tokens) {
    log("INFO", "Generating text with image: " + prompt + ", image: " + image_path + ", max_tokens: " + std::to_string(max_tokens));
...
// 多模态生成
std::string Qwen25VLInferenceEngine::generateTextWithImages(const std::string& prompt, const std::vector<std::vector<float>>& image_features, int max_tokens) {
    log("INFO", "Generating text with image features: " + prompt + ", max_tokens: " + std::to_string(max_tokens));
...
std::vector<int32_t> Qwen25VLInferenceEngine::tokenize(const std::string& text) {
    ...
    if (text == "你好") {
        ...
        if (pos != std::string::npos) {
            remaining = remaining.substr(pos + 6); // 跳过"你好"(UTF-8中每个中文字符3字节)
...
// 批量生成
std::vector<std::string> Qwen25VLInferenceEngine::generateBatch(const std::vector<std::string>& prompts) {
    log("INFO", "Generating batch of " + std::to_string(prompts.size()) + " prompts");
...
// 流式生成
void Qwen25VLInferenceEngine::generateStream(const std::string& prompt, std::function<void(const std::string&)> callback, int max_tokens) {
...
// 启用量化
bool Qwen25VLInferenceEngine::enableQuantization(const std::string& quant_type) {
...
void Qwen25VLInferenceEngine::enableParallelProcessing(bool enable) {
    ...
    log("INFO", "Parallel processing " + std::string(enable ? "enabled" : "disabled"));
}
...
// 获取模型配置
ModelConfig Qwen25VLInferenceEngine::getModelConfig() const {
    return config_;
}
...
std::string Qwen25VLInferenceEngine::detokenize(const std::vector<int32_t>& tokens) {
    std::cout << "[DEBUG] Detokenizing " << tokens.size() << " tokens" << std::endl;
    std::string result;
    for (int32_t token : tokens) {
        std::cout << "[DEBUG] Processing token: " << token << std::endl;
        if (token == bos_token_id_ || token == eos_token_id_ || token == pad_token_id_) {
            std::cout << "[DEBUG] Skipping special token: " << token << std::endl;
            continue;
        }
        std::string token_str = getTokenString(token);
        std::cout << "[DEBUG] Token " << token << " -> \"" << token_str << "\"" << std::endl;
        result += token_str;
    }
    std::cout << "[DEBUG] Final detokenized result: \"" << result << "\"" << std::endl;
    return result;
}
...
bool Qwen25VLInferenceEngine::loadVocabulary() {
    ...
    // 如果无法从GGUF加载，使用占位符实现
    std::cout << "[DEBUG] Using placeholder vocabulary with " << config_.vocab_size << " tokens" << std::endl;
...
void Qwen25VLInferenceEngine::precomputeRoPEFreqs() {
    ...
    for (size_t i = 0; i < rope_freqs_.size(); ++i) {
        rope_freqs_[i] = 1.0f / std::pow(config_.rope_theta, 2.0f * i / config_.hidden_size);
    }
}

bool Qwen25VLInferenceEngine::loadTensorFromGGUF(const std::string& tensor_name, Tensor& tensor) {
...
Tensor Qwen25VLInferenceEngine::forward(const std::vector<int32_t>& input_ids) {
    ...
    for (uint32_t i = 0; i < config_.num_layers; ++i) {
        // 注意力
        Tensor attention_output = multiHeadAttention(hidden_states, transformer_layers_[i], i);
    ...
    for (uint32_t i = 0; i < config_.vocab_size; ++i) {
        ...
        for (uint32_t j = 0; j < config_.hidden_size; ++j) {
            sum += hidden_states.data[j] * output_projection_.data[j * config_.vocab_size + i];
        }
        logits.data[i] = sum;
    }
...
Tensor Qwen25VLInferenceEngine::embedTokens(const std::vector<int32_t>& token_ids) {
    Tensor embeddings({static_cast<uint32_t>(token_ids.size()), config_.hidden_size});
...
Tensor Qwen25VLInferenceEngine::applyLayerNorm(const Tensor& input, const Tensor& weights, const Tensor& bias) {
    Tensor output = input;
...
Tensor Qwen25VLInferenceEngine::applyRoPE(const Tensor& input, uint32_t position) {
    Tensor output = input;
    
    // 简化的RoPE实现
    for (size_t i = 0; i < rope_freqs_.size() && i * 2 + 1 < input.data.size(); ++i) {
...
Tensor Qwen25VLInferenceEngine::multiHeadAttention(const Tensor& input, const TransformerLayer& layer, uint32_t layer_idx) {
...
Tensor Qwen25VLInferenceEngine::feedForward(const Tensor& input, const TransformerLayer& layer) {
    Tensor output({config_.hidden_size});
...
Tensor Qwen25VLInferenceEngine::processVisionInput(const std::vector<std::vector<float>>& image_features) {
    Tensor output({config_.hidden_size});
...
int32_t Qwen25VLInferenceEngine::sampleTemperature(const Tensor& logits, float temp) {
...
void Qwen25VLInferenceEngine::applyTemperature(Tensor& logits, float temperature) {
    for (float& val : logits.data) {
        val /= temperature;
    }
}

std::vector<std::pair<float, int32_t>> Qwen25VLInferenceEngine::getTopKTokens(const Tensor& logits, int k) {
    ...
    // 按分数降序排序
    std::sort(tokens.begin(), tokens.end(), std::greater<std::pair<float, int32_t>>());
...
float Qwen25VLInferenceEngine::calculatePerplexity(const std::vector<int32_t>& tokens) {
    // 占位符实现
    return 1.0f;
}

// SIMD优化方法
void Qwen25VLInferenceEngine::vectorAdd(const float* a, const float* b, float* result, size_t size) {
    for (size_t i = 0; i < size; ++i) {
        result[i] = a[i] + b[i];
    }
}

void Qwen25VLInferenceEngine::vectorMul(const float* a, const float* b, float* result, size_t size) {
    for (size_t i = 0; i < size; ++i) {
        result[i] = a[i] * b[i];
    }
}

void Qwen25VLInferenceEngine::matrixMultiply(const float* a, const float* b, float* c, size_t m, size_t n, size_t k) {
