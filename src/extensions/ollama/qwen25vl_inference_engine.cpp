...
Tensor Qwen25VLInferenceEngine::multiHeadAttention(
  ...
  // 验证权重大小
  size_t expected_weight_size = hidden_size * hidden_size;
  if (head.query_weights.data.size() != expected_weight_size ||
      head.key_weights.data.size() != expected_weight_size ||
      head.value_weights.data.size() != expected_weight_size) {
    log("ERROR", "multiHeadAttention: Weight size mismatch. Expected: " + 
        std::to_string(expected_weight_size) + 
        ", Q: " + std::to_string(head.query_weights.data.size()) +
  ...
  // 使用高效GEMM批量计算Q、K、V
  try {
    std::vector<const float*> inputs = {input.data.data(), input.data.data(), input.data.data()};
    std::vector<const float*> weights = {head.query_weights.data.data(), head.key_weights.data.data(), head.value_weights.data.data()};
    std::vector<float*> outputs = {q_tensor.data.data(), k_tensor.data.data(), v_tensor.data.data()};
    
    batchedGEMM(inputs, weights, outputs, 3, seq_len, hidden_size, hidden_size);
  } catch (const std::exception& e) {
    log("ERROR", "multiHeadAttention: Batched GEMM failed: " + std::string(e.what()));
    return input;
  }
