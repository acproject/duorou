...
void duorou::MacOSTray::removeMenuItem(const std::string& item_id) {
    ...
    auto it = std::find_if(menu_items_.begin(), menu_items_.end(),
                          [&item_id](const MenuItemInfo& info) {
                              return info.id == item_id;
                          });
...
void duorou::MacOSTray::setMenuItemEnabled(const std::string& item_id, bool enabled) {
...
bool duorou::MacOSTray::updateMenuItemLabel(const std::string& item_id, const std::string& label) {
    auto* info = findMenuItemInfo(item_id);
    if (info && info->menuItem) {
        info->label = label;
        @autoreleasepool {
...
void duorou::MacOSTray::addMenuItemWithId(const std::string& item_id, const std::string& label, std::function<void()> callback) {
    if (!initialized_ || !menu_) {
        return;
    }
    ...
    if (hasMenuItem(item_id)) {
        std::cerr << "Menu item with ID '" << item_id << "' already exists" << std::endl;
        return;
    }
...
void duorou::MacOSTray::setMenuItemVisible(const std::string& item_id, bool visible) {
    auto* info = findMenuItemInfo(item_id);
    if (info && info->menuItem) {
...
void duorou::MacOSTray::setMenuItemIcon(const std::string& item_id, const std::string& icon_name) {
    auto* info = findMenuItemInfo(item_id);
    if (info && info->menuItem) {
        info->icon_name = icon_name;
        @autoreleasepool {
            NSImage* icon = [NSImage imageNamed:[NSString stringWithUTF8String:icon_name.c_str()]];
            if (icon) {
...
void duorou::MacOSTray::setMenuItemShortcut(const std::string& item_id, const std::string& shortcut) {
    auto* info = findMenuItemInfo(item_id);
    if (info && info->menuItem) {
...
// 辅助方法实现
duorou::MacOSTray::MenuItemInfo* duorou::MacOSTray::findMenuItemInfo(const std::string& item_id) {
...
NSMenuItem* duorou::MacOSTray::createNSMenuItem(const MenuItemInfo& info) {
    @autoreleasepool {
        ...
        if (!info.shortcut.empty()) {
            NSString* shortcutStr = [NSString stringWithUTF8String:info.shortcut.c_str()];
            [menuItem setKeyEquivalent:shortcutStr];
        }
        
        // 设置图标
        if (!info.icon_name.empty()) {
            NSImage* icon = [NSImage imageNamed:[NSString stringWithUTF8String:info.icon_name.c_str()]];
            if (icon) {
...
void duorou::MacOSTray::updateWindowStateMenu(bool window_visible) {
    ...
    if (hasMenuItem("show_window")) {
        setMenuItemVisible("show_window", !window_visible);  // 窗口隐藏时显示"显示窗口"
    }
    if (hasMenuItem("hide_window")) {
        setMenuItemVisible("hide_window", window_visible);   // 窗口显示时显示"隐藏窗口"
    }
}
...
void duorou::MacOSTray::setQuitCallback(std::function<void()> callback) {
    ...
    if (hasMenuItem("quit")) {
        std::cout << "[MacOSTray] Found existing quit menu item, updating callback" << std::endl;
        auto* info = findMenuItemInfo("quit");
        if (info) {
            info->callback = [this]() {
                std::cout << "[MacOSTray] Quit menu item clicked!" << std::endl;
                if (quit_callback_) {
                    std::cout << "[MacOSTray] Calling quit callback function" << std::endl;
                    quit_callback_();
                } else {
                    std::cout << "[MacOSTray] Warning: quit_callback_ is null" << std::endl;
                }
            };
