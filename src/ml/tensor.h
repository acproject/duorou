...
enum class DataType { FLOAT32, FLOAT16, BF16, INT32, INT16, INT8, UINT8, BOOL };
...
public:
  ...
  ggml_tensor *to_ggml(ggml_context *ctx) const;
  // 反向： 把 ggml 计算结果拷贝会 host （FP32)场景
  void from_ggml(const ggml_tensor *src);
  ...
  template <typename T> T &at(const std::vector<int64_t> &indices);
