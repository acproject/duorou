...
// Ollama模型管理器构造函数
TextGenerator::TextGenerator(std::shared_ptr<duorou::extensions::ollama::OllamaModelManager> model_manager, const std::string &model_id)
    : context_size_(2048), vocab_size_(32000), model_manager_(model_manager), model_id_(model_id), use_ollama_(true) {
  // 初始化随机数生成器
  std::random_device rd;
  rng_.seed(rd());
}
...
GenerationResult TextGenerator::generate(const std::string &prompt,
  ...
  if (use_ollama_ && model_manager_) {
    std::cout << "[DEBUG] Using Ollama model manager for inference" << std::endl;
    ...
    try {
      ...
      if (response.success) {
        ...
        std::cout << "[DEBUG] Ollama inference successful: " << result.text.substr(0, 50) << "..." << std::endl;
      } else {
        std::cout << "[DEBUG] Ollama inference failed: " << response.error_message << std::endl;
      ...
      std::cout << "[DEBUG] Exception during Ollama inference: " << e.what() << std::endl;
    ...
    // 简单的模拟响应
    if (prompt.find("你好") != std::string::npos || prompt.find("hello") != std::string::npos) {
      result.text = "你好！我是 Duorou AI 助手，很高兴为您服务。有什么我可以帮助您的吗？";
    } else {
      result.text = "感谢您的提问。这是一个模拟的文本生成响应。当前版本使用简化的实现，未来将集成完整的 llama.cpp 功能。";
    }
    result.finished = true;
    result.stop_reason = "completed";
    result.prompt_tokens = countTokens(prompt);
  ...
  std::cout << "[DEBUG] TextGenerator returning result: " << result.text.substr(0, 30) << "..." << std::endl;
  return result;
}
...
GenerationResult TextGenerator::generateStream(const std::string &prompt,
  ...
  if (use_ollama_ && model_manager_) {
    std::cout << "[DEBUG] Using Ollama model manager for streaming inference" << std::endl;
    ...
    try {
      ...
      if (response.success) {
        ...
        std::cout << "[DEBUG] Ollama streaming inference successful" << std::endl;
      } else {
        std::string error_msg = "抱歉，流式推理过程中出现错误: " + response.error_message;
      ...
      std::string error_msg = "抱歉，流式推理过程中出现异常: " + std::string(e.what());
      callback(0, error_msg, true);
    ...
    std::cout << "[DEBUG] Using fallback mock streaming implementation" << std::endl;
    
    // 模拟流式生成
    std::string response_text;
    if (prompt.find("你好") != std::string::npos || prompt.find("hello") != std::string::npos) {
      response_text = "你好！我是 Duorou AI 助手，很高兴为您服务。有什么我可以帮助您的吗？";
    } else {
      response_text = "感谢您的提问。这是一个模拟的流式文本生成响应。当前版本使用简化的实现，未来将集成完整的 llama.cpp 功能。";
    }
...
bool TextGenerator::canGenerate() const {
  std::cout << "[DEBUG] TextGenerator::canGenerate() called - returning true (functionality enabled)" << std::endl;
  // 启用文本生成功能
  return true;
}
