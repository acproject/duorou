...
// Simple image processing functions (actual projects may need specialized image
// libraries)
...
ImageGenerator::textToImage(const std::string &prompt,
...
ImageGenerationResult
ImageGenerator::textToImageWithProgress(const std::string &prompt,
...
ImageGenerationResult ImageGenerator::imageToImage(
    const std::string &prompt, const std::vector<uint8_t> &input_image,
    int input_width, int input_height, const ImageGenerationParams &params) {
  std::lock_guard<std::mutex> lock(mutex_);
...
bool ImageGenerator::canGenerate() const { return sd_ctx_ != nullptr; }

std::string ImageGenerator::getModelInfo() const { return model_info_; }
...
double ImageGenerator::estimateGenerationTime(
    const ImageGenerationParams &params) const {
  if (generation_count_ == 0) {
    // 基于经验的估算
    double base_time = 2.0;                   // 基础时间（秒）
    double step_factor = params.steps / 20.0; // 步数因子
    double size_factor =
        (params.width * params.height) / (512.0 * 512.0); // 尺寸因子
...
bool ImageGenerator::saveImage(const ImageGenerationResult &result,
  ...
  if (format == "png") {
    success = stbi_write_png(file_path.c_str(), result.width, result.height,
                             result.channels, result.image_data.data(),
                             result.width * result.channels);
  } else if (format == "jpg" || format == "jpeg") {
    success = stbi_write_jpg(file_path.c_str(), result.width, result.height,
                             result.channels, result.image_data.data(),
                             90); // 质量90%
  } else if (format == "bmp") {
    success = stbi_write_bmp(file_path.c_str(), result.width, result.height,
                             result.channels, result.image_data.data());
  }
...
std::vector<uint8_t>
ImageGenerator::resizeImage(const std::vector<uint8_t> &image_data,
  ...
  int success = stbir_resize_uint8(image_data.data(), input_width, input_height,
                                   0, output_data.data(), output_width,
                                   output_height, 0, channels);
...
std::string ImageGenerator::preprocessPrompt(const std::string &prompt) const {
  ...
  // 移除多余的空格
  processed.erase(
      std::unique(processed.begin(), processed.end(),
...
bool ImageGenerator::validateImageSize(int width, int height) const {
  ...
  // 检查尺寸范围
  if (width < 64 || height < 64 || width > max_size_.first ||
      height > max_size_.second) {
    return false;
  }
...
ImageGenerationResult
ImageGenerator::convertSDImage(sd_image_t *sd_image) const {
  ImageGenerationResult result;
...
ImageGeneratorFactory::create(sd_ctx_t *sd_ctx) {
  if (!sd_ctx) {
    return nullptr;
  }
