...
// 将任意字节序列转换为合法的 UTF-8 字符串：
// - 已经是合法 UTF-8 的片段原样保留（包括中文等多字节字符）
// - 非法起始字节或孤立续字节会被转义为 "<0xHH>"
static std::string sanitizeToValidUTF8(const std::string &s) {
  std::string out;
  size_t i = 0;
  auto hex = [](unsigned char b) {
    static const char *digits = "0123456789ABCDEF";
    std::string r = "<0x";
    r.push_back(digits[(b >> 4) & 0xF]);
    r.push_back(digits[b & 0xF]);
    r.push_back('>');
    return r;
  };
  ...
  while (i < s.size()) {
    ...
    // Determine expected length by leading bits
    size_t need = 0;
    if ((c & 0xE0) == 0xC0)
      need = 2; // 110xxxxx
    else if ((c & 0xF0) == 0xE0)
      ...
      need = 4; // 11110xxx
    else {      // Invalid leading or continuation byte at start
      out += hex(c);
      ++i;
      continue;
    }
    ...
    // Not enough bytes or bad continuation: escape current byte and move on
    out += hex(c);
    ++i;
  }
  return out;
...
void Vocabulary::initialize(const std::vector<std::string> &values,
  ...
  // Clear cached data (once_flag cannot be reset, so we clear the cached
  // results)
...
std::vector<int32_t>
Vocabulary::addSpecials(const std::vector<int32_t> &ids) const {
  std::vector<int32_t> result = ids;

  // Add BOS token if needed
  if (addBOS_ && !bos_.empty()) {
    if (result.empty() ||
        std::find(bos_.begin(), bos_.end(), result[0]) == bos_.end()) {
  ...
  if (addEOS_ && !eos_.empty()) {
    if (result.empty() ||
        std::find(eos_.begin(), eos_.end(), result.back()) == eos_.end()) {
  ...
  if (id >= 0 && static_cast<size_t>(id) < values_.size()) {
    ...
    if (token_text.length() == 6 && token_text.substr(0, 3) == "<0x" &&
      ...
      try {
        ...
        // If hex parsing fails, fall back to returning token as-is
        return token_text;
      }
    }

    // Handle placeholder tokens like <token_146895>
    // These often represent byte values that cannot be directly encoded as
    // UTF-8
    if (token_text.length() > 7 && token_text.substr(0, 7) == "<token_" &&
        token_text.back() == '>') {
      ...
      try {
        ...
        if (token_id >= 0 && token_id <= 255) {
          unsigned char b = static_cast<unsigned char>(token_id);
          return std::string(1, static_cast<char>(b));
        } else if (token_id > 255) {
          unsigned char b = static_cast<unsigned char>(token_id % 256);
          return std::string(1, static_cast<char>(b));
        }
      } catch (const std::exception &) {
        // If parsing fails, return raw token text
        return token_text;
      }
    }

    // For regular tokens, return the token text as-is (raw bytes).
    // We avoid per-token sanitization to preserve multi-byte UTF-8 sequences
...
int Vocabulary::getMergeRank(const std::string &left,
...
void Vocabulary::setPAD(const std::vector<int32_t> &pad) { pad_ = pad; }

void Vocabulary::setUNK(const std::vector<int32_t> &unk) { unk_ = unk; }
...
void Vocabulary::buildSpecialTokens() const {
  specialTokens_.clear();
  for (size_t i = 0; i < values_.size(); ++i) {
    if (i < types_.size() && (types_[i] == TOKEN_TYPE_CONTROL ||
                              types_[i] == TOKEN_TYPE_USER_DEFINED)) {
...
std::string Vocabulary::decodeText(const std::string &text) const {
  ...
  while (i < text.length()) {
    ...
    if (byte < 0x80) {
      ...
      if (i + 1 < text.length()) {
        codepoint =
            ((byte & 0x1F) << 6) | (static_cast<uint8_t>(text[i + 1]) & 0x3F);
        i += 2;
      } else {
        i++;
      }
  ...
  for (uint32_t cpt : codepoints) {
    ...
    if (cpt >= 0x21 && cpt <= 0x7E) {
      ...
      // This is a simplified mapping - in practice, GPT-2 uses a specific
      // mapping table
      byte_val = static_cast<uint8_t>(cpt - 256);
    } else {
      // For other code points, use the code point value directly if it fits in
      // a byte
      byte_val = static_cast<uint8_t>(cpt);
    }
