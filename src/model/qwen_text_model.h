...
public:
  ...
  std::vector<float> forward(duorou::ml::Context &ctx,
  ...
  bool loadWeights(duorou::extensions::ollama::GGUFParser &parser,
                   size_t layerIndex);
...
private:
  ...
  // Whether to apply RoPE inside attention (true) or at embedding stage (false)
  bool applyRopeInAttention_ = false;
  // Lazy-init flag: whether MHA has been bound with actual weights
  // (allocated/attached)
...
public:
  ...
  std::vector<int32_t> generate(const std::vector<int32_t> &inputIds,
                                size_t maxLength = 512,
                                float temperature = 1.0f,
                                float topP = 0.9f) override;
  ...
  duorou::ml::Tensor forward(duorou::ml::Context &ctx,
  ...
  // Step-by-step decode: compute logits for the last token, optionally using KV
  // Cache
  duorou::ml::Tensor stepDecode(duorou::ml::Context &ctx,
                                const duorou::ml::Tensor &lastTokenId,
                                duorou::kvcache::Cache *cache = nullptr);

  // New: nextToken helper using stepDecode with temperature and top-p sampling
  int32_t nextToken(duorou::ml::Context &ctx,
                    const duorou::ml::Tensor &lastTokenId,
                    duorou::kvcache::Cache *cache, float temperature,
                    float topP);
  ...
  std::vector<float>
  applyPositionalEncoding(const std::vector<float> &embeddings,
  ...
  // Bind an external vocabulary/tokenizer from upper-level (e.g., multimodal
  // model) Ensures getVocabSize() reflects external vocab and logits size
  // matches expected.
...
private:
  ...
  int32_t sampleToken(const std::vector<float> &probabilities,
