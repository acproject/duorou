...
namespace duorou {
// Map duorou::std to the global ::std to avoid nested-namespace resolution
// issues
namespace std = ::std;
namespace model {
...
QwenMultimodalModel::QwenMultimodalModel(
    const QwenMultimodalConfig &config,
...
bool QwenMultimodalModel::loadModel(const std::string &modelPath) {
  if (!initialized_) {
    std::cerr << "[WARN] Model not initialized; proceeding with minimal "
                 "GGUF-based setup"
              << std::endl;
  ...
  // If a GGUF file is passed in, prioritize initializing vocabulary and
  // tokenizer from GGUF
  ...
  if (!modelPath.empty() && (ends_with(modelPath, ".gguf") ||
                             modelPath.find(".gguf") != std::string::npos)) {
    gguf_inited = loadGGUFModel(modelPath);
    if (!gguf_inited) {
      std::cerr << "[WARN] Failed to initialize tokenizer/vocab from GGUF: "
                << modelPath << std::endl;
    }
  } else {
    ...
    if (probe.parseFile(modelPath)) {
      std::cout
          << "[DEBUG] Probed GGUF successfully for path (no .gguf extension): "
          << modelPath << std::endl;
      gguf_inited = loadGGUFModel(modelPath);
      if (!gguf_inited) {
        std::cerr
            << "[WARN] Failed to initialize tokenizer/vocab from probed GGUF: "
            << modelPath << std::endl;
      }
      ...
      if (probe2.parseFile(config_.textModelPath)) {
        std::cout << "[DEBUG] Probed GGUF successfully for config path: "
                  << config_.textModelPath << std::endl;
        gguf_inited = loadGGUFModel(config_.textModelPath);
        if (!gguf_inited) {
          std::cerr << "[WARN] Failed to initialize tokenizer/vocab from "
                       "probed GGUF: "
                    << config_.textModelPath << std::endl;
        }
      }
  ...
  if (textModel_) {
    if (external_vocabulary_) {
      if (!textModel_->initialize(config_.configPath, true)) {
        std::cerr
            << "[WARN] Failed to initialize text model with external vocabulary"
            << std::endl;
      }
    } else {
      if (!textModel_->initialize(config_.configPath)) {
        std::cerr << "[WARN] Failed to initialize text model without external "
                     "vocabulary"
                  << std::endl;
      }
    }
    ...
    std::cerr
        << "[ERROR] textModel_ is null after minimal setup; cannot proceed"
        << std::endl;
    return false;
  }

  // In minimal setup, skip loading vision model/components to avoid null
  // dereference
  return true;
}

std::vector<int32_t> QwenMultimodalModel::encode(const std::string &text,
                                                 bool addSpecial) {
  std::cout << "[DEBUG] QwenMultimodalModel::encode called with text: '" << text
            << "' (length: " << text.length()
            << " bytes, addSpecial: " << addSpecial << ")" << std::endl;

  // If external vocabulary exists, prioritize using tokenizer created based on
  // external vocabulary
  if (external_vocabulary_) {
    ...
    if (tokenizer_) {
      std::cout << "[DEBUG] Tokenizer is available, using proper tokenization"
                << std::endl;
      ...
      std::cout
          << "[WARN] Tokenizer is NULL! Falling back to text model encoding"
          << std::endl;
      ...
      if (!textModel_) {
        std::cerr << "[ERROR] Both tokenizer and text model are unavailable"
                  << std::endl;
        return {};
      }

      std::vector<int32_t> result = textModel_->encode(text, addSpecial);
      std::cout << "[DEBUG] Text model fallback encoded " << result.size()
                << " tokens" << std::endl;
      return result;
    }
  }

  std::cout << "[DEBUG] No external vocabulary, falling back to text model"
            << std::endl;
  ...
  std::vector<int32_t> result = textModel_->encode(text, addSpecial);
  std::cout << "[DEBUG] Text model encoded " << result.size() << " tokens"
            << std::endl;
  return result;
}
...
PixelValues
QwenMultimodalModel::processPixelValues(const std::vector<uint8_t> &imageData) {
  if (!imageProcessor_) {
  ...
  PixelValues result =
      PixelValues::fromRawData(processedData, height, width, channels);
...
std::vector<int32_t>
QwenMultimodalModel::encodeMultimodal(const MultimodalInputData &input) {
...
std::vector<int32_t>
QwenMultimodalModel::postTokenize(const std::vector<int32_t> &tokenIds) {
...
duorou::ml::Tensor
QwenMultimodalModel::forward(duorou::ml::Context &ctx,
  ...
  if (inputIds.numel() > 0) {
    if (inputIds.dtype() != duorou::ml::DataType::INT32) {
      std::cerr
          << "[WARN] QwenMultimodalModel::forward expected INT32 inputIds but "
             "got different dtype; attempting to interpret as INT32"
          << std::endl;
    }
    inputIds_vec.resize(static_cast<size_t>(inputIds.numel()));
    inputIds.copyToHost(inputIds_vec.data(),
                        inputIds_vec.size() * sizeof(int32_t));
  }

  // 若存在视觉特征，则按占位符位置融合到文本嵌入
  if (!visionFeatures.empty()) {
    // 提取序列ID到主机
    std::vector<int32_t> ids;
    ids.resize(static_cast<size_t>(inputIds.numel()));
    inputIds.copyToHost(ids.data(), ids.size() * sizeof(int32_t));

    // 构造覆盖掩码：在 visionPadId 位置置 true
    std::vector<bool> mask(ids.size(), false);
    size_t padCount = 0;
    for (size_t i = 0; i < ids.size(); ++i) {
      if (ids[i] == config_.visionPadId) {
        mask[i] = true;
        ++padCount;
      }
    }

    // 将视觉特征映射到文本隐藏维度，长度为 fusedTokens * hidden
    const size_t textHidden =
        static_cast<duorou::model::QwenTextModel *>(textModel_.get())
            ->getHiddenSize();
    std::vector<float> external(textHidden * ids.size(), 0.0f);

    // 视觉特征数量可能少于占位符数量（例如2x2合并后为 patch/4），按最小值铺设
    size_t fusedRows = visionFeatures.size() / textHidden;
    size_t needed = std::min(fusedRows, padCount);
    size_t copied = 0;
    for (size_t i = 0; i < ids.size() && copied < needed; ++i) {
      if (!mask[i])
        continue;
      const float *src = &visionFeatures[copied * textHidden];
      float *dst = &external[i * textHidden];
      std::copy(src, src + textHidden, dst);
      ++copied;
    }
    ...
    auto logitsVec =
        static_cast<duorou::model::QwenTextModel *>(textModel_.get())
            ->forwardWithExternalEmbeddings(ctx, ids, external, mask, cache);

    // 转为Tensor返回
    std::vector<int64_t> shape = {static_cast<int64_t>(logitsVec.size())};
    duorou::ml::Tensor out =
        duorou::ml::Tensor::zeros(shape, duorou::ml::DataType::FLOAT32);
    float *outData = out.data<float>();
    if (outData && !logitsVec.empty()) {
      std::copy(logitsVec.begin(), logitsVec.end(), outData);
    }
    return out;
  }

  // 无视觉特征：直接文本前向
  return textModel_->forward(ctx, inputIds, cache);
}

std::vector<int32_t> QwenMultimodalModel::generateMultimodal(
    const std::vector<int32_t> &inputIds,
    const std::vector<PixelValues> &pixelValues, size_t maxLength,
...
bool QwenMultimodalModel::loadFromCheckpoint(
...
bool QwenMultimodalModel::initializeComponents() {
  // If external vocabulary is not yet provided, but the configured text model
  // path is a GGUF file, then prioritize initializing external_vocabulary_ and
  // tokenizer_ from GGUF
  if (!external_vocabulary_) {
    const std::string &path = config_.textModelPath;
    if (!path.empty() && path.find(".gguf") != std::string::npos) {
      bool ok = loadGGUFModel(path);
      if (ok) {
        std::cout << "[DEBUG] Initialized external vocabulary from GGUF in "
                     "initializeComponents, size="
                  << (external_vocabulary_ ? external_vocabulary_->size() : 0)
                  << std::endl;
      } else {
        std::cerr
            << "[WARN] Failed to initialize from GGUF in initializeComponents: "
            << path << std::endl;
      }
    }
  }

  // If external vocabulary exists, no need to initialize text model's
  // vocabulary
  if (external_vocabulary_) {
    // Use external vocabulary, create a text model without initializing
    // vocabulary
    textModel_ = std::make_unique<QwenTextModel>(config_.textOptions);
    // Call textModel_->initialize() to set initialized_ flag, but skip
    ...
    // Bind external vocabulary to text model so its getVocabSize() matches
    // external vocab
    textModel_->setExternalVocabulary(external_vocabulary_);
    std::cout << "[DEBUG] Using external vocabulary with size: "
              << external_vocabulary_->size() << std::endl;

    // Create tokenizer based on external vocabulary (using Qwen architecture
    // factory) ONLY if not already created
    if (!tokenizer_) {
      TokenizerFactoryOptions
          opts; // Allow future override through configuration
      tokenizer_ = createTextProcessorForArchitecture(
          "qwen", external_vocabulary_, opts);
      if (!tokenizer_) {
        std::cerr
            << "[ERROR] Failed to create tokenizer with external vocabulary"
            << std::endl;
      } else {
        std::cout << "[DEBUG] Tokenizer created via architecture factory using "
                     "external vocabulary"
                  << std::endl;
      }
    } else {
      std::cout << "[DEBUG] Reusing tokenizer created from GGUF; skip "
                   "architecture factory creation"
                << std::endl;
    }
  } else {
    // Only create text model instance, defer initialization to
    // loadComponentModels stage, avoid fallback vocabulary initialization logs
    textModel_ = std::make_unique<QwenTextModel>(config_.textOptions);
    std::cout
  ...
  // imageProcessor_ =
  // std::make_unique<QwenImageProcessor>(config_.imageProcessorConfig);

  // Don't take ownership of text model's vocabulary here, avoid double
  // free/dangling pointers QwenMultimodalModel::getVocabulary() will safely
  // return external_vocabulary_ or textModel_->getVocabulary()
...
bool QwenMultimodalModel::loadComponentModels() {
  ...
  if (!external_vocabulary_) {
    ...
    std::cout << "[DEBUG] Skipping textModel_ initialize/loadModel because "
                 "external_vocabulary_ is provided"
              << std::endl;
    // Even when external vocabulary is provided, we still need to load real
    // weights
    if (!config_.textModelPath.empty()) {
      bool ok = textModel_->loadModel(config_.textModelPath);
      if (!ok) {
        std::cerr << "[WARN] Failed to load text model weights from: "
                  << config_.textModelPath << std::endl;
        success = false;
      } else {
        std::cout << "[DEBUG] Loaded text model weights from GGUF: "
                  << config_.textModelPath << std::endl;
      }
    } else {
      std::cout << "[WARN] textModelPath is empty; using zero-initialized "
                   "weights which may degrade output quality"
                << std::endl;
    }
  }
...
std::vector<int32_t>
QwenMultimodalModel::addVisionTokens(const std::vector<int32_t> &tokens,
...
std::vector<float> QwenMultimodalModel::processVisionFeatures(
    const std::vector<PixelValues> &pixelValues) {
  if (!visionModel_) {
    return {};
  }
  ...
  for (const auto &pv : pixelValues) {
    ...
    for (size_t i = 0; i < tensorData.size(); ++i) {
      imageData[i] = static_cast<uint8_t>(
          std::clamp(tensorData[i] * 255.0f, 0.0f, 255.0f));
    }
...
bool QwenMultimodalModel::loadConfig(const std::string &configPath) {
  ...
  if (isGGUFPath) {
    ...
    std::cout << "[DEBUG] Detected GGUF model path; using GGUF fallback for "
                 "Qwen multimodal config: "
              << configPath << std::endl;
    return true;
  }
  ...
  if (!in.is_open()) {
    std::cerr << "[ERROR] Failed to open Qwen multimodal config: " << configPath
              << std::endl;
    return false;
  }

  // SAFETY: Avoid exceptions from nlohmann::json (vendor may map JSON_THROW to
  // std::abort). Parse with allow_exceptions = false to prevent SIGABRT on
  // malformed JSON.
  std::stringstream ss;
  ss << in.rdbuf();
  ...
  nlohmann::json j = nlohmann::json::parse(
      content, nullptr, /*allow_exceptions=*/false, /*ignore_comments=*/false);
  if (j.is_discarded()) {
    ...
    try {
      ...
      if (parser.parseFile(configPath)) {
        config_.textModelPath = configPath;
        std::cout << "[WARN] Config JSON malformed; detected GGUF. Falling "
                     "back to GGUF: "
                  << configPath << std::endl;
        return true;
      }
    ...
    std::cerr
        << "[ERROR] Failed to parse Qwen multimodal config (malformed JSON): "
        << configPath << std::endl;
    return false;
  }
  ...
  try {
    ...
    config_.visionModelPath =
        j.value("vision_model_path", config_.visionModelPath);

    // Text options
    if (j.contains("text_options") && j["text_options"].is_object()) {
      const auto &jt = j["text_options"];
      config_.textOptions.hiddenSize =
          jt.value("hidden_size", config_.textOptions.hiddenSize);
      config_.textOptions.numHeads =
          jt.value("num_heads", config_.textOptions.numHeads);
      config_.textOptions.numKVHeads =
          jt.value("num_kv_heads", config_.textOptions.numKVHeads);
      config_.textOptions.ropeDim =
          jt.value("rope_dim", config_.textOptions.ropeDim);
      config_.textOptions.originalContextLength = jt.value(
          "original_context_length", config_.textOptions.originalContextLength);
      config_.textOptions.eps = jt.value("eps", config_.textOptions.eps);
      config_.textOptions.ropeBase =
          jt.value("rope_base", config_.textOptions.ropeBase);
      config_.textOptions.ropeScale =
          jt.value("rope_scale", config_.textOptions.ropeScale);
      config_.textOptions.blockCount =
          jt.value("block_count", config_.textOptions.blockCount);
      config_.textOptions.embeddingLength =
          jt.value("embedding_length", config_.textOptions.embeddingLength);
    }

    // Vision options
    if (j.contains("vision_options") && j["vision_options"].is_object()) {
      const auto &jv = j["vision_options"];
      config_.visionOptions.hiddenSize =
          jv.value("hidden_size", config_.visionOptions.hiddenSize);
      config_.visionOptions.numHeads =
          jv.value("num_heads", config_.visionOptions.numHeads);
      config_.visionOptions.numLayers =
          jv.value("num_layers", config_.visionOptions.numLayers);
      config_.visionOptions.patchSize =
          jv.value("patch_size", config_.visionOptions.patchSize);
      config_.visionOptions.imageSize =
          jv.value("image_size", config_.visionOptions.imageSize);
      config_.visionOptions.numChannels =
          jv.value("num_channels", config_.visionOptions.numChannels);
      config_.visionOptions.temporalPatchSize = jv.value(
          "temporal_patch_size", config_.visionOptions.temporalPatchSize);
      config_.visionOptions.spatialMergeSize = jv.value(
          "spatial_merge_size", config_.visionOptions.spatialMergeSize);
      config_.visionOptions.layerNormEps =
          jv.value("layer_norm_eps", config_.visionOptions.layerNormEps);
    }

    // Image processor config
    if (j.contains("image_processor") && j["image_processor"].is_object()) {
      const auto &jp = j["image_processor"];
      config_.imageProcessorConfig.imageSize =
          jp.value("image_size", config_.imageProcessorConfig.imageSize);
      config_.imageProcessorConfig.patchSize =
          jp.value("patch_size", config_.imageProcessorConfig.patchSize);
      config_.imageProcessorConfig.temporalPatchSize =
          jp.value("temporal_patch_size",
                   config_.imageProcessorConfig.temporalPatchSize);
      config_.imageProcessorConfig.spatialMergeSize = jp.value(
          "spatial_merge_size", config_.imageProcessorConfig.spatialMergeSize);
      config_.imageProcessorConfig.minPixels =
          jp.value("min_pixels", config_.imageProcessorConfig.minPixels);
      config_.imageProcessorConfig.maxPixels =
          jp.value("max_pixels", config_.imageProcessorConfig.maxPixels);
      config_.imageProcessorConfig.resampleMode =
          jp.value("resample_mode", config_.imageProcessorConfig.resampleMode);
      config_.imageProcessorConfig.doResize =
          jp.value("do_resize", config_.imageProcessorConfig.doResize);
      config_.imageProcessorConfig.doNormalize =
          jp.value("do_normalize", config_.imageProcessorConfig.doNormalize);
      config_.imageProcessorConfig.doConvertRgb =
          jp.value("do_convert_rgb", config_.imageProcessorConfig.doConvertRgb);
      ...
      if (jp.contains("mean") && jp["mean"].is_array()) {
        ...
        for (const auto &v : jp["mean"]) {
          if (v.is_number_float()) {
            config_.imageProcessorConfig.mean.push_back(
                static_cast<float>(v.get<double>()));
          } else if (v.is_number_integer()) {
            config_.imageProcessorConfig.mean.push_back(
                static_cast<float>(v.get<int64_t>()));
          } else {
            std::cerr << "[WARN] image_processor.mean contains non-numeric "
                         "value; skipping"
                      << std::endl;
          }
        }
      ...
      if (jp.contains("std") && jp["std"].is_array()) {
        ...
        for (const auto &v : jp["std"]) {
          if (v.is_number_float()) {
            config_.imageProcessorConfig.std.push_back(
                static_cast<float>(v.get<double>()));
          } else if (v.is_number_integer()) {
            config_.imageProcessorConfig.std.push_back(
                static_cast<float>(v.get<int64_t>()));
          } else {
            std::cerr << "[WARN] image_processor.std contains non-numeric "
                         "value; skipping"
                      << std::endl;
          }
        }
    ...
    if (j.contains("special_tokens") && j["special_tokens"].is_object()) {
      ...
      config_.visionStartId =
          js.value("vision_start_id", config_.visionStartId);
    ...
    if (j.contains("processing") && j["processing"].is_object()) {
      ...
      config_.maxImageTokens =
          jp.value("max_image_tokens", config_.maxImageTokens);
      config_.maxSequenceLength =
          jp.value("max_sequence_length", config_.maxSequenceLength);
      config_.useVisionPadding =
          jp.value("use_vision_padding", config_.useVisionPadding);
    }

    return true;
  } catch (const std::exception &e) {
    std::cerr << "[ERROR] Exception while parsing Qwen multimodal config: "
              << e.what() << std::endl;
    return false;
  }
}
...
std::vector<size_t> QwenMultimodalModel::findImageTokenPositions(
...
size_t
QwenMultimodalModel::calculateImageTokenCount(const PixelValues &pixelValues) {
  // 基于patch数并考虑2x2合并后的token数
  size_t patchCount = pixelValues.totalPatches();
  size_t merge =
      std::max<size_t>(1, config_.visionOptions.spatialMergeSize *
                              config_.visionOptions.spatialMergeSize);
  size_t mergedTokens = patchCount / merge;
  // 使用配置上限
  return std::min(mergedTokens, config_.maxImageTokens);
}

bool QwenMultimodalModel::isSpecialToken(int32_t tokenId) const {
  return tokenId == config_.imageTokenId || tokenId == config_.videoTokenId ||
         tokenId == config_.visionStartId || tokenId == config_.visionEndId ||
         tokenId == config_.visionPadId;
}

std::vector<int32_t> QwenMultimodalModel::getSpecialTokens() const {
  return {config_.imageTokenId, config_.videoTokenId, config_.visionStartId,
          config_.visionEndId, config_.visionPadId};
}
...
createQwenMultimodalModel(const std::string &configPath) {
...
MultimodalInputData
createMultimodalInput(const std::string &text,
...
size_t estimateTokenCount(const MultimodalInputData &input,
...
std::vector<int32_t>
mergeTokenSequences(const std::vector<std::vector<int32_t>> &sequences) {
...
splitTokenSequence(const std::vector<int32_t> &tokens, int32_t separatorToken) {
...
PixelValues PixelValues::fromRawData(const std::vector<float> &rawData,
  ...
  {
    duorou::ml::Backend *backend =
        duorou::ml::BackendManager::getInstance().getCurrentBackend();
    if (backend) {
...
bool QwenMultimodalModel::initializeMLComponents() {
  try {
    ...
    kvCache_ = std::make_unique<duorou::kvcache::CacheWrapper>(
        duorou::kvcache::CacheType::CAUSAL);

    // Initialize attention using configuration-driven parameters
    // Embed dim, heads, and kv-heads derive from textOptions; head_dim =
    // hiddenSize / numHeads
    const int64_t embed_dim =
        static_cast<int64_t>(config_.textOptions.hiddenSize);
    const int64_t num_heads =
        static_cast<int64_t>(config_.textOptions.numHeads);
    const int64_t kv_heads =
        static_cast<int64_t>(config_.textOptions.numKVHeads);
    ...
    std::cerr << "Failed to initialize ML components: " << e.what()
              << std::endl;
    return false;
  }
}
...
duorou::ml::Tensor
QwenMultimodalModel::convertToTensor(const std::vector<int32_t> &data) {
  std::vector<int64_t> shape = {static_cast<int64_t>(data.size())};
  duorou::ml::Tensor tensor(shape, duorou::ml::DataType::INT32);
  // Backend-aware allocation: prefer model Context backend, fallback to global
  // current backend
  {
    duorou::ml::Backend *backend = nullptr;
    if (mlContext_)
      backend = mlContext_->getBackend();
    if (!backend)
      backend = duorou::ml::BackendManager::getInstance().getCurrentBackend();
    if (backend)
...
duorou::ml::Tensor
QwenMultimodalModel::convertToTensor(const std::vector<float> &data,
...
std::vector<float>
QwenMultimodalModel::convertFromTensor(const duorou::ml::Tensor &tensor) {
...
bool QwenMultimodalModel::loadGGUFModel(const std::string &modelPath) {
  try {
    // Parse GGUF file
    ggufParser_ =
        std::make_unique<duorou::extensions::ollama::GGUFParser>(true);
    if (!ggufParser_->parseFile(modelPath)) {
      std::cerr << "Failed to load GGUF model: parseFile failed for "
                << modelPath << std::endl;
      return false;
    }

    // Create Vocabulary from GGUF using unified factory (same as
    // tokenizer_golden_test.cpp)
    ...
    std::cout << "[DEBUG] External vocabulary set from GGUF, size="
              << external_vocabulary_->size() << std::endl;

    // Create TextProcessor (Tokenizer) from GGUF
    TokenizerFactoryOptions opts; // defaults; can be extended via config
    tokenizer_ =
        createTextProcessorFromGGUF(*ggufParser_, external_vocabulary_, opts);
    if (!tokenizer_) {
      std::cerr << "[ERROR] Failed to create tokenizer from GGUF" << std::endl;
      return false;
    }
    std::cout << "[DEBUG] Tokenizer created successfully from GGUF (vocab_size="
    ...
    std::cout << "[DEBUG] GGUF tokenizer sanity roundtrip: '" << sanity_text
              << "' -> ids(" << sanity_ids.size() << ") -> '" << sanity_decoded
              << "'" << std::endl;
...
duorou::ml::Tensor
QwenMultimodalModel::loadTensorFromGGUF(const std::string &tensorName) {
  if (!ggufParser_) {
  ...
  auto mapDType =
      [](duorou::extensions::ollama::GGMLTensorType t) -> duorou::ml::DataType {
    ...
    default:
      return duorou::ml::DataType::INT8; // store raw bytes for quantized types
    }
  };
  ...
  // If bytes do not match allocation (e.g., quantized layouts), fall back to
  // raw flat buffer
  if (tensor.nbytes() != fileBytes) {
    ...
    duorou::ml::Tensor raw({static_cast<int64_t>(fileBytes)},
                           duorou::ml::DataType::INT8);
    if (backend)
      raw.setBackend(backend);
    raw.allocate(backend);
    if (!ggufParser_->readTensorData(*tensorInfo, raw.data(), fileBytes)) {
      std::cerr << "[ERROR] Failed to read GGUF tensor data: " << tensorName
                << std::endl;
      return duorou::ml::Tensor();
    }
    return raw;
